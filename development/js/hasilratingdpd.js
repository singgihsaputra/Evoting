...
function getDaftarDPD(x) {
	$('#ulDPD').html('');
    $.get('http://localhost/server/hasilratingDPD', 'id='+x).done(function (result) {
        console.log(result);
        var a = '';
        for (i = 0; i < result.length; i++) {
            a = a + '<li><a onclick=detail('+result[i].id_dpd+')><img src=' + urlimg + result[i].foto + '><h2>' + result[i].nama + '</h2><p>' + result[i].domisili + '</p><p class="ui-li-aside"><strong>Rating : '+result[i].rating+'</strong></p></a></li>';
        }
        $('#ulDPD').html(a);
        $('ul').listview('refresh');
    }).fail(function () {
